function MediaPlayBanner(p)
{
	...
	this.createBanner = function(){
        ...
        //设置文字
        $("#bannerTitle").text(params.title ? params.title : "");
    ...
    this.getMplayerStatus = function () {
        self.duration = params.mp.mpGetDuration(false);
        self.curTime = params.mp.mpGetCurTime(false);
        $("#bannerStartTime").text("00:00:00");
        $("#bannerEndTime").text(transformTime(self.duration));
        $("#playTime").text(transformTime(self.curTime));
        processbar_blue_width = 790 * (self.curTime / self.duration);
        $("#processbar_blue_bg").css('width', processbar_blue_width + 'px');

        if(self.curTime == 0 && self.fastTowardFlag != 0){
            self.fastTowardFlag = 0;
            self.fastTowardNum = 0;
            self.videoResume();
        }

        if(self.curTime == self.duration && self.fastTowardFlag != 0){
            self.fastTowardFlag = 0;
            self.fastTowardNum = 0;
            self.videoResume();
        }
    ...
    this.showDetail = function () {
        ...
        var html = '<div id="ch_mediaplay_details">';
            ...
            html += '<span>'+Lp.getValue("Close")+'</span>';
            html += '</div>';
        ...
        $("#content_title").text(params.title);
    ...
    this.languageSwitch = function ()
    {
        ...
        if(params.item.audio.length > 1) {
            self.languageNum++;
            if(self.languageNum >= params.item.audio.length){
                self.languageNum = 0;
            }
        }
        else {
        ...
        $("#languageTxt").text(Lp.getValue("audio_track") + (self.languageNum + 1) + ' : ' + getCountryByCode(params.item.audio[self.languageNum].lang));

        if(params.item.audio.length > 1){
            var p = {
                "aStreamType": params.item.audio[self.languageNum].type,
                "pid":params.item.audio[self.languageNum].pid
            };
    ...
    this.fastForward = function ()
    {
        ...
        if(self.fastTowardFlag <= 0){
            console.log("fastBackward -> fastForward");
            self.fastTowardNum = 0;
    ...
    this.fastBackward = function ()
    {
        ...
        if(self.fastTowardFlag >= 0){
            console.log("fastForward -> fastBackward");
            self.fastTowardNum = 0;
            self.fastTowardFlag = -1;
        }
    ...
    this.dragBackward = function ()
    {
        ...
        console.log("curTime:"+self.curTime+"  dragTime:"+dragTime +"   pos:"+pos);
        params.mp.mpSeek(pos,false);
